...
- User `!` commands on the user-shell PTY now forward keystrokes to the running command, so an interactive prompt can be answered instead of blocking until its deadline. A wizard's arrow-key picker, a pager, or a `sudo` password prompt rendered correctly but received nothing, because the bash execution component never handled input and the live PTY session was never exposed to the caller that owns the UI. This includes a command fired while the model is streaming — that is precisely when an interactive prompt would otherwise sit unanswerable — and focus returns to the composer once the run ends, unless a later `!` has claimed it since. While forwarding is live the running label reads `keys go to the command`, so the capability is visible rather than something to guess at. Esc cancels the run — while that component holds focus the input controller's own Esc handler is unreachable — and keys are dropped once the command completes so a keystroke racing the exit cannot reach a dead session. The non-PTY fallback (the bash snapshot path, or `PI_NO_PTY=1`) has no session to write to and stays display-only.
- A user `!` command whose keystrokes are being forwarded no longer inherits the model bash tool's deadline. Driving an interactive command by hand takes as long as the person takes, so a wizard left open past `tools.maxTimeout` (5 minutes by default) was killed with `Command timed out after 300 seconds` while every keypress was in fact reaching it. Runs that expose no PTY session keep their deadline, since nothing can answer or cancel those.
- A user `!` command's streaming PTY output now requests a repaint of its own, instead of marking itself dirty and waiting for an unrelated render. Only `render()` consumes that flag, so a new frame reached the screen on the loader spinner's tick — and under output backlog the render was deferred further still. An interactive command therefore appeared not to respond to keystrokes at all: an arrow key moved the selection in the command immediately, but the redrawn frame surfaced seconds later, which is indistinguishable from input never being delivered.
- A user `!` command's output block now inherits the session's tool-output expansion state, so the `… N more lines (ctrl+o to expand)` hint acts on the first press. With expansion already enabled, a new bash/eval block still rendered collapsed and kept advertising the hint, while the press it advertised turned expansion *off* — leaving the block collapsed and requiring a second press to actually reveal it. Replayed bash/eval blocks were affected on session restore, and in the agent transcript viewer they were never registered as expandable at all, so the viewer's expand key could not reach them.
- Tool result output no longer repaints SIXEL image lines: an inline image in a tool's text result had its payload wrapped in the theme's color escapes and rendered as garbage instead of a picture. Image lines now pass through untouched while the surrounding text keeps its own colors.
